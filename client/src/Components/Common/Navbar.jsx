...
const NavBar = () => {
  ...
  return (
    <div className="w-full h-[72px] fixed z-[1000] transition-all duration-300">
      <div className="flex justify-between items-center w-11/12 mx-auto h-full px-4 md:px-10">
            ...
            <div className="flex flex-col leading-none">
              <div className="flex items-center gap-1">
                <span className="text-[13px] font-black uppercase tracking-[0.2em] text-white">
                  Fund <span className="text-[#F97316]">India</span>
                </span>
              </div>
        ...
        <ul className="hidden lg:flex gap-x-8 border border-white/10 bg-white/5 backdrop-blur-xl px-12 py-2.5 rounded-2xl">
          {NavbarLinks.map((link, index) => (
            <li key={index} className="relative group">
              {link?.title === "Services" ? (
                <div className="flex gap-x-1 items-center cursor-pointer text-white hover:text-[#F97316] transition-all">
                  <p className="text-sm font-medium">{link.title}</p>
                  <MdKeyboardArrowDown />
                  <div className="absolute top-10 left-1/2 -translate-x-1/2 w-56 bg-[#0a0a0a] border border-white/10 rounded-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 p-2 shadow-2xl z-50">
                    <div className="absolute -top-1.5 left-1/2 -translate-x-1/2 w-3 h-3 bg-[#0a0a0a] border-t border-l border-white/10 rotate-45" />
                    {SERVICE_ITEMS.map((item, i) => (
                      <Link
                        key={i}
                        to={item.path}
                        className="block p-3 text-xs font-bold uppercase tracking-widest text-[#9ca3af] hover:text-[#F97316] hover:bg-white/5 rounded-xl transition-all"
                      >
                        {item.label}
                      </Link>
                    ))}
          ...
          <div className="hidden md:flex items-center gap-x-4">
            {token === null ? (
              <>
                <Link
                  to="/login"
                  className="bg-white/5 border border-white/10 text-white px-5 py-2 rounded-xl text-sm font-bold uppercase tracking-widest hover:bg-white/10 transition-all flex items-center gap-2"
                >
                  <CiLogin size={18} /> Login
                </Link>
                <Link
                  to="/signup"
                  className="bg-white text-black px-5 py-2 rounded-xl text-sm font-bold uppercase tracking-widest hover:bg-[#F97316] transition-all"
                >
                  Sign Up
                </Link>
              </>
              ...
              <div className="relative group">
                {avatarSrc ? (
                  ...
                  <div className="h-9 w-9 rounded-full border border-[#F97316] cursor-pointer flex items-center justify-center text-xs font-bold text-white bg-white/5">
                    {avatarInitial}
                  </div>
                )}
                <div className="absolute right-0 top-12 w-48 bg-[#0a0a0a] border border-white/10 rounded-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 p-2 shadow-2xl z-50">
                  <button
                    onClick={() => navigate("/dashboard")}
                    className="w-full flex items-center gap-3 p-3 text-xs font-bold uppercase text-[#9ca3af] hover:text-white hover:bg-white/5 rounded-xl transition-all"
                  >
          ...
          {token && (
            <div className="flex items-center gap-4 p-4 bg-white/5 rounded-2xl border border-white/5 mb-6">
              {avatarSrc ? (
                ...
                <div className="h-10 w-10 rounded-full border border-[#F97316] flex items-center justify-center text-xs font-bold text-white bg-white/5">
                  {avatarInitial}
                </div>
              )}
                ...
                {link.title === "Services" ? (
                  <>
                    <button
                      ...
                      className="flex items-center justify-between text-xl font-bold tracking-tighter text-white w-full py-2 transition-colors hover:text-[#F97316]"
                    >
                      {link.title}
                      <FiChevronDown
                      ...
                      <div className="pl-4 flex flex-col gap-4 mt-3 mb-3 border-l border-white/10 animate-fadeDown">
                        {SERVICE_ITEMS.map((item, idx) => (
                          <Link
                            ...
                            to={item.path}
                            onClick={() => setIsMenuOpen(false)}
                            className="font-bold uppercase tracking-widest text-gray-300 text-[0.7rem] hover:text-[#F97316]"
                          >
                            * {item.label}
                          </Link>
                        ))}
            ...
            {token && (
              <Link
                ...
                className={`text-xl font-bold tracking-tighter flex items-center gap-3 ${
                  mathroute("/dashboard") ? "text-[#F97316]" : "text-white"
                }`}
              >
            ...
            {token === null ? (
              <>
                ...
                <Link
                  to="/signup"
                  onClick={() => setIsMenuOpen(false)}
                  className="w-full py-4 text-center bg-white rounded-2xl text-black font-bold uppercase tracking-widest text-xs hover:bg-[#F97316] transition-all"
                >
                  Initialize Access
                </Link>
              </>
